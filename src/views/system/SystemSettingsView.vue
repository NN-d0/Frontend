<template>
  <div class="page-container">
    <el-row :gutter="16">
      <el-col :span="8">
        <el-card class="overview-card" shadow="hover">
          ...
          <div class="metric-sub">当前重点展示告警阈值相关配置项</div>
        </el-card>
      </el-col>
      ...
      <el-col :span="8">
        <el-card class="overview-card" shadow="hover">
          ...
          <div class="metric-sub">用于高功率异常告警判定</div>
        </el-card>
      </el-col>
      ...
      <el-col :span="8">
        <el-card class="overview-card" shadow="hover">
          ...
          <div class="metric-sub">用于低信噪比异常告警判定</div>
        </el-card>
      </el-col>
    </el-row>
      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            <el-tag type="warning">配置键修改后可能影响告警阈值生效</el-tag>
          </div>
        </div>
      </template>
    ...
    <el-dialog v-model="dialogVisible" title="修改告警阈值" width="620px">
      <el-alert
        title="提示：当前后端告警阈值逻辑默认读取固定配置键，若修改配置键名称，可能导致阈值不再生效。"
        type="warning"
        :closable="false"
        show-icon
      ...
      <el-form ref="formRef" :model="editForm" :rules="rules" label-width="100px">
        <el-form-item label="配置名称" prop="configName">
          <el-input v-model="editForm.configName" placeholder="请输入配置名称" />
        </el-form-item>

        <el-form-item label="配置键" prop="configKey">
          <el-input v-model="editForm.configKey" placeholder="请输入配置键" />
        </el-form-item>
...
const handleSave = async () => {
  ...
  if (!valid) {
    ElMessage.warning('请完整填写配置名称、配置键、配置值')
    return
  }

  if (!editForm.id) {
    ElMessage.warning('未获取到配置ID')
    return
  }

  if (!String(editForm.configName ?? '').trim()) {
    ElMessage.warning('配置名称不能为空')
    return
  }

  if (!String(editForm.configKey ?? '').trim()) {
  ...
  try {
    ...
    await updateConfigApi({
      id: editForm.id,
      configName: String(editForm.configName).trim(),
      configKey: String(editForm.configKey).trim(),
