  ...
  <div class="page-container">
    ...
    <el-card class="page-card query-card" shadow="hover">
      ...
      <div class="quick-tags">
        ...
        <el-tag class="click-tag" effect="plain" @click="setQuickStatus('')">全部</el-tag>
        <el-tag class="click-tag" type="danger" effect="plain" @click="setQuickStatus(0)">未处理</el-tag>
        <el-tag class="click-tag" type="warning" effect="plain" @click="setQuickStatus(1)">已确认</el-tag>
        <el-tag class="click-tag" type="success" effect="plain" @click="setQuickStatus(2)">已处理</el-tag>
      </div>
    </el-card>
      ...
      <template #header>
        <div class="card-header">
          <div>
            ...
            <div class="header-subtitle">统一使用 /api/core/alarms/page</div>
          </div>
        </div>
      </template>
      ...
      <div v-if="currentAlarm" class="detail-shell">
        <div class="detail-top-card">
          ...
          <div class="detail-top-tags">
            <el-tag :type="alarmLevelTag(currentAlarm.alarmLevel)">
              {{ currentAlarm.alarmLevel || '-' }}
            </el-tag>
            <el-tag :type="alarmStatusTag(currentAlarm.alarmStatus)">
              {{ alarmStatusText(currentAlarm.alarmStatus) }}
            </el-tag>
          </div>
        </div>
...
const handleConfirm = async (row) => {
  try {
    await ElMessageBox.confirm(
      `确认将告警【${row.alarmNo || row.id}】标记为已确认吗？`,
      '确认告警',
      {
        type: 'warning'
      }
...
const submitHandle = async () => {
  if (!handleForm.alarmId) {
    ElMessage.warning('未获取到告警ID')
    return
  }

  if (!handleForm.handleNote) {
